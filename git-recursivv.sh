  ...
  if git -C "$repo_dir" cat-file -e "$commit:.gitmodules" 2>/dev/null; then
    ...
    for submoudle in "${submodules[@]}"; do
      ...
      if [[ "$submodule_url" =~ ^\.\./ ]]; then
        local parent_url=$(git -C "$repo_dir" config --get remote.origin.url)
        parent_url="${parent_url%.git}"
        local depth=$(echo "$submodule_url" | grep -o '\.\./' | wc -l)
        for i in $(seq 1 $depth); do
          parent_url=$(dirname "$parent_url")
        done
        local sub_path=$(echo "$submodule_url" | sed 's|\.\./||g')
        submodule_url="${parent_url}/${sub_path}"
      fi
